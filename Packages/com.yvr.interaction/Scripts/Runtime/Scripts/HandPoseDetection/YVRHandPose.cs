...
namespace YVR.Interaction
{
    public class YVRHandPose : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (trackType == TrackType.Right || trackType == TrackType.Any)
            {
                ...
                rightShapesHold = ShapesRecognizerCheck(rightJointPos, rightWirstRot * Vector3.left,
                    rightWirstRot * Vector3.forward, rightWirstRot * Vector3.up);
        ...
        {
            if (angle > (min -width) && angle < (max+width))
            {
        ...
        private Vector3 GetTrackAxis(TrackType trackType, Quaternion wristRot)
        {
            switch (transTrackAxis)
            {
                case TransRecognizer.TrackAxis.Fingers:
                    palmAxis = wristRot * Vector3.back;
                    break;
                case TransRecognizer.TrackAxis.Palm:
                    palmAxis = wristRot * Vector3.down;
                    break;
                case TransRecognizer.TrackAxis.Thumb:
                    palmAxis = trackType == TrackType.Left ? wristRot * Vector3.left : wristRot * Vector3.right;
                    break;
            }
